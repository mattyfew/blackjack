...
app.post('tsrh', function (req, res) {
	db.run('INSERT INTO threads (title,subtitle,author_id,content) VALUES (?,?,?,?)',
		req.body.thread_title, req.body.thread_subtitle, req.body.author_name, req.body.thread_content,
		function (err, rows) { //need these values!
			if (err) {
				throw err;
			} else {
				res.redirect('/tsrh');
